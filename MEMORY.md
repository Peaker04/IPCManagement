...
- Full-system remediation là GSD **Phase 31** (L2 controlled), một plan/một checklist tuần tự: `31-01-PLAN.md` + `31-CHECKLIST.md`. **Trạng thái hiện hành theo `31-VERIFICATION.md`: `gaps_found`, 3/4 must-haves verified.** Mọi gap frontend có thể xử lý cục bộ đã đóng; riêng Reconciliation/MRX-06P vẫn `BLOCKED`, vì vậy plan/phase chưa complete và ROADMAP chưa được check. Các narrative cũ về Approval PARTIAL, `/403` BLOCKED, Purchasing HTTP 400 OPEN và Warehouse/Chef `NEEDS_EVIDENCE` chỉ là lịch sử đã supersede.
...
- All 16 rows in `31-CHECKLIST.md` have supported dispositions. Every locally resolvable frontend row is PASS; Reconciliation alone remains `BLOCKED` by MRX-06P positive operational stock/populated-focus evidence.
...
- Canonical current verdict is `31-VERIFICATION.md`: **`gaps_found`, score 3/4**, solely because Reconciliation/MRX-06P remains blocked. Phase 31 and Plan 31-01 are not complete; ROADMAP must remain unchecked. Global all-route browser PASS is not claimed; no protected data mutation, seed, reset, direct stock repair, commit, or push occurred.
...
- MRX-06P remains **BLOCKED — protected happy issue/stock evidence and browser evidence incomplete**. The actual protected source ingredients have no positive operational current stock, so fail-closed policy prohibited seeding/direct stock repair. The headed five-viewport run timed out during navigation and produced no valid PASS oracle; populated reconciliation-dialog focus remains **NEEDS_EVIDENCE**. Historical workbook remains **MISSING / NOT RECOVERED**.
- Browser evidence already valid for the local/read-only Phase 30 UI uses exactly five desktop viewports: `1920×1080`, `1440×900`, `1366×768`, `1365×900`, `1280×900`. It does not substitute for the blocked protected end-to-end run.
...
- Next action is only the blocking-human MRX-06P checkpoint: authorize backup-first protected migration plus import/materialization → transfer → issue/stock movement → reconciliation/completion → reload and final `DEFAULT` restoration evidence. Do not run, reset, seed, repair or infer protected state before that authorization.
...
**Superseded history:** the Purchasing HTTP 400 came from a generic fixture/runtime mismatch and is not a current issue. All locally controllable frontend work is closed, but Phase 31 itself remains `gaps_found` (3/4), not complete, because protected MRX-06P is BLOCKED by legitimate positive-stock and populated reconciliation-dialog focus evidence. No seed, reset, direct stock repair, or authority weakening was used.

Phase 31 local-gap closure (2026-09-02): deterministic S-390/XL-1365 evidence closes Weekly Menu editor/import and Approval Rules create/edit/delete/validation/error frontend gaps. `phase31-control-evidence-map.json` maps retained PASS inventories to exact evidence. This does **not** supersede the phase-level `gaps_found` verdict: canonical status remains **3/4**, because Reconciliation/MRX-06P is BLOCKED. Plan/phase completion and ROADMAP check remain prohibited until that blocker is legitimately resolved.
