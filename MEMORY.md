...
- Phase 30 evidence-backed UI/UX correction is implemented: Weekly Menu `view/customerId/weekStartDate` is URL-authoritative, transfer fails closed without an exact `menuVersionId`, Warehouse preserves tab semantics and exposes one prerequisite action, Reconciliation drawers use the canonical focus-trapping dialog, and retained surfaces use shared Vietnamese status/unit presentation. Admin BOM import/action alignment and Admin Audit filter vocabulary were corrected surgically; DEFAULT composition and business authority remain unchanged. The stale Admin Data source-contract expectation was aligned with the existing reconciliation guard. Focused UI contracts now pass 18/18, backend capability/eligibility passes 12/12, ESLint, production build (2,312 modules), checklist and diff hygiene pass. Runtime dialog focus with a populated batch and schedule-empty recovery remain `NEEDS_EVIDENCE`/open rather than claimed PASS.
