...
- Additive migration `20260828092012_ClosedLoopReconciliationIssueLineage` is now applied to the canonical local `ipcmanagement` database after a verified backup at `.artifacts/runtime/phase30-migration-20260828-183213/`. EF history, `binary(16)` lineage columns, indexes and readiness migration check passed. It remains unapplied to protected `ipc_lane7`; protected mutation/evidence still requires fresh authorization.
...
- Weekly Menu mode-separation gap fix: the page now routes to a dedicated `ReconciliationWeeklyMenuPage` before any DEFAULT hooks/query owners mount. Reconciliation exposes exactly `Kế hoạch tuần` and `Định lượng xuất kho`; transfer controls mount only in the latter. Warehouse and Reconciliation remain dedicated compositions, while Admin Data now routes to a BOM/audit-only model that does not mount cleanup, employee, inventory or statistics owners. The system-operation provider fails closed with a visible version/configuration error when a stale backend publishes an incomplete reconciliation capability instead of rendering partial tabs. DEFAULT composition remains unchanged. Verification: focused 7 files / 31 tests, ESLint, production build (2,311 modules), checklist and diff hygiene pass.
- Headed Chrome follow-up found and fixed composition leaks: reconciliation Dashboard does not mount/preload legacy workflow-report owners; reconciliation Admin Data does not request customer contracts; and its retained Audit tab is explicitly backend-authorized without restoring Reports navigation. Final five-viewport headed recheck (`1920×1080`, `1440×900`, `1366×768`, `1365×900`, `1280×900`) captured exact `MATERIAL_RECONCILIATION` capabilities, URL-owned demand deep links, matching selected tabpanels, zero prohibited Coordination requests, API/console/page/request failures, and zero document overflow. Evidence: `.artifacts/runtime/phase30-live-mode-audit/recheck-final/evidence.json` plus screenshots.
- Phase 30 evidence-backed UI/UX correction is implemented: Weekly Menu `view/customerId/weekStartDate` is URL-authoritative, transfer fails closed without an exact `menuVersionId`, Warehouse preserves tab semantics and exposes one prerequisite action, Reconciliation drawers use the canonical focus-trapping dialog, and retained surfaces use shared Vietnamese status/unit presentation. Admin BOM import/action alignment and Admin Audit filter vocabulary were corrected surgically; DEFAULT composition and business authority remain unchanged. The stale Admin Data source-contract expectation was aligned with the existing reconciliation guard. Schedule now distinguishes missing customer, missing week and settled ready-empty plans outside the generic table row, with one valid focus action and no DEFAULT import/query owner. Five-viewport headed evidence for missing-scope and ready-empty states is `.artifacts/runtime/phase30-live-mode-audit/schedule-empty-2026-08-28T18-13-00-283Z/evidence.json`: both recovery states and focus actions pass 5/5 with zero API/console/page/request failures or document overflow. Reconciliation composition now uses explicit `QueryViewBoundary` geometry roles: compact scope/state boundaries no longer inherit a 380px table placeholder, empty batch state hides the impossible empty select and provides one actionable `Mở định lượng xuất kho` surface. Five-viewport composition evidence is `.artifacts/runtime/phase30-live-mode-audit/reconciliation-composition-2026-08-28T18-52-54-212Z/evidence.json`: compact min-height is `0px`, one empty surface is visible, no batch select is rendered, action is visible, and API/console/page/request/overflow checks are clean 5/5. Runtime dialog focus with a populated batch remains `NEEDS_EVIDENCE`; it is not claimed PASS.
- Four screenshot-raised composition defects are corrected at their owners. Reconciliation Dashboard now presents the workflow as four compact, equal-height step cards instead of oversized status rows; the populated reconciliation Weekly Menu matrix owns the remaining viewport (`ipc-weekly-menu-shell--viewport-fill`) instead of stopping at the global 520px cap and leaving blank page space; Advanced Settings removes repeated navigation/group explanatory notes; and operation mode is a two-card selector with one explicit active mode and one switch action. Five-viewport headed evidence is `.artifacts/runtime/phase30-live-mode-audit/ui-composition-four-fixes-2026-08-29T00-29-16-279Z/evidence.json`: dashboard has 4 cards at 114–134px, Weekly Menu bottom gap is 1.8125px, settings has exactly 2 mode cards / 1 active card / 0 visible note paragraphs, and API/console/page/request/overflow checks are clean 5/5. Failed diagnostic attempts remain preserved and are not PASS evidence.
- Plan `30-03` Task 1 is complete. Plan `30-04` is an already-executed PARTIAL prerequisite/handoff with no forward blocker: verified exact-family return/correction, DEFAULT-only supplemental and explicit legacy truths/commits are preserved, while dependent Plans `30-05..08` own the remaining MRX-04 inactive-owner matrix. Executor sequencing is numeric waves 4 through 13 (`30-04` through `30-13`), while product narrative remains backend conceptual Wave 4 and frontend conceptual Wave 5. Plan `30-05` is test-first retain-only for the InventoryIssueService fence already present at base `f4b47b40`; production changes require a real failing executable test. Local migration and five-viewport composition recheck are complete, but populated mutation/dialog evidence is not. Protected Task 2 remains paused until Kỳ explicitly authorizes the backup-first `ipc_lane7` checkpoint.
