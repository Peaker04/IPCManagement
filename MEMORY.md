...
- Headed Chrome follow-up found and fixed two remaining composition leaks: the reconciliation Dashboard no longer mounts or preloads legacy workflow-report owners, and reconciliation Admin Data no longer requests excluded customer contracts. Fresh 1440×900 coverage of Dashboard, Weekly Menu, Warehouse, Reconciliation and Admin Data has zero prohibited Coordination requests, console/page/request errors or horizontal overflow; Weekly Menu displays both required tabs and Warehouse/Reconciliation show explicit missing-batch prerequisites. Evidence: `.artifacts/runtime/phase30-live-mode-audit/evidence.json` plus screenshots. Local API 5262 and Vite 5173 were launched from current source by this audit. The local database still lacks migration `20260828092012_ClosedLoopReconciliationIssueLineage`; it was not applied.
- Phase 30 evidence-backed UI/UX correction is implemented without backend or database changes: Weekly Menu `view/customerId/weekStartDate` is URL-authoritative, transfer fails closed without an exact `menuVersionId`, Warehouse preserves tab semantics and exposes one prerequisite action, Reconciliation drawers use the canonical focus-trapping dialog, and retained surfaces use shared Vietnamese status/unit presentation. Admin BOM import/action alignment and Admin Audit filter vocabulary were corrected surgically; DEFAULT composition and business authority remain unchanged. Focused UI contracts 11/11, ESLint, production build (2,312 modules), checklist and diff hygiene pass. A broader admin-data source-contract run still has one pre-existing expectation mismatch for the already-enabled reconciliation contracts guard and is not attributed to this diff.
