...
- Chuỗi chuẩn hóa bảng dùng `docs/TABLE-STANDARDIZATION-ROADMAP.md` làm contract thứ tự Wave 0–8 và checklist đóng wave; current execution đang ở Wave 3 (ledger/document dài hạn). Import-history đã batch rollback eligibility theo page (4 query cố định), FE đã truyền active customer scope/reset page khi scope đổi, regression khóa date window inclusive qua 2023→2030, OpenAPI/FE giữ DateOnly không timezone-shift, rollback giữ nguyên list page, SQL regression xác nhận DB `LIMIT`, và fixture chứng minh bytes giảm theo page size trên history nhiều năm. Consumer scan sáu shared ledger owners đã có production/test consumers. Wave 3 còn carry-over `W3-CARRY-01`: production-like MySQL `EXPLAIN`/index evidence, giao cho Wave 6; không suy luận query plan production từ SQLite. `docs/CODE-CLEANUP-INVENTORY.md` giữ candidate dead/unwired; hai script chưa có consumer (`generate-status-tokens.mjs`, `measure-ultimate-baseline.mjs`) chỉ ở trạng thái REVIEW, không được chạy/xoá/commit đoán mò trong dirty worktree.
- Chuỗi chuẩn hóa bảng dùng `docs/TABLE-STANDARDIZATION-ROADMAP.md` làm contract thứ tự Wave 0–8 và checklist đóng wave; current execution đang ở Wave 4 preparation sau khi Wave 3 đã đủ page/payload/date/context evidence và giữ carry-over `W3-CARRY-01` cho Wave 6. Wave 4 audit đầu tiên bỏ duplicate `BOM áp dụng 100%` khỏi Admin Contracts ContextStrip, giữ fact tại cột bảng; focused render test pass. `docs/CODE-CLEANUP-INVENTORY.md` giữ candidate dead/unwired; hai script chưa có consumer (`generate-status-tokens.mjs`, `measure-ultimate-baseline.mjs`) chỉ ở trạng thái REVIEW, không được chạy/xoá/commit đoán mò trong dirty worktree.
- Chuỗi chuẩn hóa bảng dùng `docs/TABLE-STANDARDIZATION-ROADMAP.md` làm contract thứ tự Wave 0–8 và checklist đóng wave; current execution đang ở Wave 4 preparation sau khi Wave 3 đã đủ page/payload/date/context evidence và giữ carry-over `W3-CARRY-01` cho Wave 6. Admin Contracts đã bỏ duplicate BOM summary và tách effective range khỏi contract status; focused render test pass. Gate kéo theo cũng sửa history pagination từ effect reset sang scope-keyed state, loại lint render-cascade; lint và 8 focused tests pass. `docs/CODE-CLEANUP-INVENTORY.md` giữ candidate dead/unwired; hai script chưa có consumer (`generate-status-tokens.mjs`, `measure-ultimate-baseline.mjs`) chỉ ở trạng thái REVIEW, không được chạy/xoá/commit đoán mò trong dirty worktree.
- Chuỗi chuẩn hóa bảng dùng `docs/TABLE-STANDARDIZATION-ROADMAP.md` làm contract thứ tự Wave 0–8 và checklist đóng wave; current execution đang ở Wave 4. Admin Contracts đã bỏ duplicate BOM summary, tách effective range khỏi status và bỏ copy technical. BOM current/preview được giữ là state của import workflow nhưng floorplan/conditional fixtures đã bỏ technical nested-tab assumptions; read-only inventory còn 15 owner, 14 focused contract test và lint pass. `frontend/tests/ui-audit.spec.ts` còn BOM-tab cleanup trong working tree nhưng chưa commit vì file có hunk copy ngoài scope đã tồn tại trước; owner phải hợp nhất an toàn. `docs/CODE-CLEANUP-INVENTORY.md` giữ candidate dead/unwired; hai script chưa có consumer (`generate-status-tokens.mjs`, `measure-ultimate-baseline.mjs`) chỉ ở trạng thái REVIEW, không được chạy/xoá/commit đoán mò trong dirty worktree.
