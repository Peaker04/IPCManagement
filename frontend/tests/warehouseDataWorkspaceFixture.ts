...
export const stockMovementRows = Array.from({ length: 8 }, (_, index) => ({
  movementId: `movement-phase27-${index + 1}`, warehouse: 'Kho chính', ingredient: currentStockRows[index].ingredient,
  movementType: index % 2 ? 'ISSUE' : 'RECEIPT', quantity: index + 0.5, unit: 'kg',
  occurredAt: `2026-08-${String(10 + index).padStart(2, '0')}T09:00:00Z`, sourceDocumentCode: `PX-P27-${String(index + 1).padStart(3, '0')}`,
  reason: index % 2 ? 'Xuất nguyên liệu theo ca phục vụ' : 'Nhập từ đơn mua đã duyệt',
}));
