...
export function resolveClassAwareAuthority(cwd:string,path=CLASS_AWARE_MARKER){
  if(path!==CLASS_AWARE_MARKER) throw new Error('class-aware marker path is not allowlisted'); const marker=JSON.parse(readFileSync(resolve(cwd,path),'utf8'));
  if(marker.planId!=='27.1-03S'||marker.status!=='COMPLETE'||marker.authority!=='CLASS_AWARE_SNAPSHOT_RECOVERY'||!Array.isArray(marker.validatorPins)||marker.validatorPins.length!==2||!Array.isArray(marker.authorityRoots)||marker.authorityRoots.length!==9||new Set(marker.authorityRoots.map((r:any)=>r.type)).size!==9) throw new Error('class-aware marker content mismatch');
  const commit=markerCommit(cwd,path,marker); for(const p of ['frontend/tests/validateVisualReconciliation.ts','frontend/tests/validateVisualReconciliation.test.ts']){const pin=marker.validatorPins.find((x:any)=>x.path===p);if(!pin||sha256(execFileSync('git',['show',`${marker.payloadCommit}:${p}`],{cwd}))!==pin.sha256||git(cwd,'rev-parse',`${marker.payloadCommit}:${p}`)!==pin.gitBlobId) throw new Error('class-aware validator pin mismatch');}
