...
function main() {
  ...
  if(mode==='downstream-reconciliation'){
    const allowed=new Set(['--mode','--validator-authority','--general-validator-values-from','--recovery-values-from','--partial-commit','--require-partial-disposition','--require-validator-test-pins','--require-eight-distinct-roots','--reject-01C-authority','--identity-manifest','--dispositions','--identity-filter','--identity-set','--identity-sets','--all-21','--require-corrected-non-stale','--require-two-packets','--verify-row-class-path-matrix','--check-allowlist','--enforce-admin-lock','--enforce-purchasing-lock','--phase-base-commit-from','--cumulative-git-state','--cumulative-authorization-union','--wave-base-commit-from','--wave-git-state','--wave-authorization-union','--pre-phase-dirty-from','--verify-frozen-infrastructure-allowlist','--verify-before-after-sha256','--recompute-invariants','--reject-planning-transition-before-pass','--output','--require-sealed-final-gate','--derive-vrec-04-from-manifests']); assertExactFlags(args,allowed);
