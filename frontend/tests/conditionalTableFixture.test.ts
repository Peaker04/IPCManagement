...
describe('Wave 2 conditional table read-only fixture', () => {
  it('covers the 13 read-only conditional table owners exactly once', () => {
    expect(conditionalTableFixtures).toHaveLength(13)
    expect(new Set(conditionalTableFixtures.map((fixture) => fixture.id)).size).toBe(13)
    expect(new Set(conditionalTableFixtures.map((fixture) => `${fixture.sourceFile}:${fixture.tableIndex}`)).size).toBe(13)
