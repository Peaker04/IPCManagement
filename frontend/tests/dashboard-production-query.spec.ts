...
test.describe('Phase 28 Dashboard production-route query-state adapter', () => {
  test('records 98 honest six-part identities from endpoint-specific GET interception', async ({ page }) => {
    ...
    for (const viewport of UI_AUDIT_VIEWPORTS) for (const state of ['initial-loading','populated','truly-empty','no-results','error-no-data'] as const) {
      ...
      for (const regionId of ['dashboard-shift-status','dashboard-workflow-exceptions']) {
        ...
        const seriousAttribution=await page.evaluate((violations)=>violations.flatMap(violation=>violation.nodes.map(node=>{
          ...
          return {ruleId:violation.id,selector,owner:element?.closest<HTMLElement>('[data-ui-owner]')?.dataset.uiOwner??null,foreground:style?.color??null,background:style?.backgroundColor??null,failureSummary:node.failureSummary};
        })),serious.map(({id,nodes})=>({id,nodes:nodes.map(({target,failureSummary})=>({target,failureSummary}))})));
        const finding=(ruleId:string,pass:boolean,value:Record<string,unknown>,expected:string)=>routeMeasuredFinding({ruleId,identity,productionRouteMeasured:true,passed:pass,measured:{...value,state,endpointInterception:true},expected,actual:JSON.stringify(value),lowestOwner:'DashboardPage'});
    ...
    const verdictTotals=records.flatMap(r=>r.findings).reduce<Record<string,number>>((a,f)=>(a[f.verdict]=(a[f.verdict]??0)+1,a),{}); const output=resolve(process.env.UI_AUDIT_OUTPUT_ROOT ?? resolve(process.cwd(),'test-results'),'ui-audit-phase28-dashboard-query-states.json'); mkdirSync(dirname(output),{recursive:true}); writeFileSync(output,`${JSON.stringify({schemaVersion:UI_AUDIT_SCHEMA_VERSION,identityCount:98,measuredIdentityCount:63,notApplicableIdentityCount:7,needsEvidenceIdentityCount:28,reasons:DASHBOARD_QUERY_DISPOSITION_REASONS,verdictTotals,records},null,2)}\n`);
  });
});
