...
describe('Phase 28 Wave 2 architecture boundary', () => {
  ...
  it('retains active-first and preserve-visited regression authorities outside the production delta', () => {
    expect(readFileSync(resolve(repositoryRoot, 'frontend/tests/browser/purchasing-production-query.spec.ts'), 'utf8')).toContain('active-first preserve-visited');
    expect(readFileSync(resolve(repositoryRoot, 'frontend/tests/browser/approvals-production-query.spec.ts'), 'utf8')).toContain('active-first visited panels');
