...
import type { OperationalKpiSummaryDto } from '@/api/workflowApiTypes';
...
export const dashboardApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getOperationalKpis: builder.query<OperationalKpiSummaryDto, void>({
      ...
      providesTags: ['Coordination', 'PurchaseOrders', workflowCacheTags.operationalKpis, ...workflowOperationalKpiSourceTags],
    }),
  }),
  overrideExisting: false,
});

export const { useGetOperationalKpisQuery } = dashboardApi;
