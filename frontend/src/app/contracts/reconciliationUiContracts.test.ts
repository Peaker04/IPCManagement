...
describe('material reconciliation UI contracts', () => {
  ...
  it('keeps warehouse tab semantics and the prerequisite action unambiguous', () => {
    ...
    expect(warehouseSource).toContain("activeView === 'demand' && batch?.status === 'IN_PROGRESS' && hasLinkedIssue && <Button")
    expect(warehouseSource).toContain("disabled={hasLinkedIssue}")
    expect(warehouseSource).toContain("'Đã xuất đủ'")
    expect(warehouseSource).toContain('Xuất thêm nguyên liệu')
