...
import { OperationalFrame, TabContentSkeleton, ViewSwitcher } from '@/components/common';
...
const AdminPanelFallback = (
  <TabContentSkeleton
    columns={6}
    rows={6}
    message="Đang tải dữ liệu quản trị..."
  />
);

function DefaultAdminDataPage() {
  const model = useAdminDataPageModel();
  const { adminTabs, effectiveActiveView, isViewPending, setActiveView, startViewTransition } = model;
        ...
        {adminTabs.length === 0 ? (
          <section className="rounded-lg border border-slate-200 bg-white p-6">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Mở Thiết lập nâng cao để khôi phục một tab được chế độ hiện tại cho phép.
            </p>
            <Link className="ipc-button ipc-button-primary mt-4" to={ROUTES.ADVANCED_SETTINGS}>
          ...
          <>
            <ViewSwitcher
              compact
              ariaLabel="Chọn vùng dữ liệu quản trị"
              tabs={adminTabs}
            ...
            {isViewPending ? (
              <span className="sr-only" role="status">
                Đang chuyển vùng dữ liệu quản trị.
              </span>
            ) : null}
