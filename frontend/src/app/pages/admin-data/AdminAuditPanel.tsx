                ...
                <tbody>
                  {displayLogs.map((log) => {
                    ...
                    const cells: Record<string, React.ReactNode> = {
                      ...
                      newValue: <div className="flex items-center justify-between gap-2"><span className="ipc-admin-audit-value" title={presentation.newValueTitle}><span className="block font-bold text-slate-900">{log.eventCode ?? presentation.after}{log.eventLineCount != null ? ` · ${log.eventLineCount} dòng` : ''}</span>{log.eventId ? <span className="mt-0.5 block text-caption font-medium text-slate-500">{eventStatusLabels[log.eventStatus ?? ''] ?? 'Trạng thái chưa xác định'} · {log.eventRole === 'UNKNOWN' ? 'Vai trò chưa được lưu' : log.eventRole}</span> : isReconciliationMode && log.businessArea === 'Issue' ? <span className="mt-0.5 block text-caption font-medium text-amber-700">Dòng chi tiết · Liên kết sự kiện chưa xác định</span> : null}</span>{isReconciliationMode && log.eventId && log.reconciliationBatchId && <Button type="button" variant="outline" size="xs" aria-label={`Xem chi tiết sự kiện ${log.eventCode ?? log.eventId}`} onClick={() => setSelectedEvent(log.eventId, log.reconciliationBatchId)}><Eye size={14} aria-hidden="true" /> Chi tiết</Button>}</div>,
