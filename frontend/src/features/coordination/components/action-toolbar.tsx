...
import { AlertTriangle, Edit, FileDown, Lock } from 'lucide-react'
...
export function ActionToolbar({ status }: { status?: string }) {
  ...
  const isBusy = isLocking || isExporting
  ...
  const confirmDialogCopy = confirmationAction === 'lock'
    ? {
      ...
      <Dialog open={confirmationAction !== null} onOpenChange={closeConfirmationDialog}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              type="button"
              onClick={confirmationAction === 'lock' ? handleLock : handleExportExcel}
              disabled={isBusy}
            >
