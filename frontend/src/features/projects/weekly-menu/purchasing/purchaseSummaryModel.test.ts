...
describe('purchase summary model', () => {
  it('keeps nonaggregate allocation fallback theoretical, not a physical issue result', () => {
    ...
    expect(result.usesDemand).toBe(false)
    expect(result.demandRows).toEqual([])
  })
  ...
  it('places shortage exceptions before sufficient materials across pages', () => {
    const enough = Array.from({ length: 12 }, (_, index) => ({
      projection: 'physical-handoff', remainingToIssueQty: 0, id: `enough-${index}`, material: `Đủ ${index}`, required: 1, available: 5, reserved: 0, unit: 'kg', source: '', status: 'Đủ', nextAction: '', tone: 'success',
    })) as DemandLine[]
    const shortage = { projection: 'physical-handoff', remainingToIssueQty: 5, id: 'shortage', material: 'Thiếu gạo', required: 5, available: 0, reserved: 0, unit: 'kg', source: '', status: 'Thiếu', nextAction: '', tone: 'danger' } as DemandLine
