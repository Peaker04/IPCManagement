...
export const WeeklyMenuImportDialog = ({ workflow, surface = 'dialog' }: { workflow: WeeklyMenuImportWorkflow; surface?: 'dialog' | 'page' }) => {
  ...
  const content = (
        <DialogContent scrollMode={surface === 'page' ? 'content' : 'body'} role={surface === 'page' ? 'region' : 'dialog'} aria-modal={surface === 'page' ? false : undefined} aria-label="Nhập thực đơn từ Excel" className={surface === 'page' ? 'max-h-none max-w-none gap-0 border-0 p-0 shadow-none' : 'ipc-weekly-dialog max-w-6xl gap-0'}>
          ...
          <DialogBody className={surface === 'page' ? 'mt-5 flex-none flex flex-col gap-5 overflow-visible pr-1' : 'mt-5 flex flex-col gap-5 pr-1'}>
            ...
            {state.rollbackTarget !== null && (
              <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-red-300 bg-red-50 p-3 text-sm text-red-950">
