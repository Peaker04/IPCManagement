...
function LocationProbe() {
  ...
  return <output aria-label="Vị trí hiện tại">{`${location.pathname}|${String(location.state?.from ?? '')}`}</output>
}
...
describe('SessionTimeoutModal expired-session recovery', () => {
  ...
  it('blocks the current route and returns to login with the deep-link origin preserved', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/reports?view=purchase']}>
        <SessionTimeoutModal />
        <LocationProbe />
      </MemoryRouter>,
    )
    ...
    await waitFor(() => expect(screen.getByLabelText('Vị trí hiện tại')).toHaveTextContent('/login|/reports'))
