...
export const TabContentSkeleton = memo(function TabContentSkeleton({
  ...
  return (
    ...
    >
      ...
      {/* Structured Table Skeleton */}
      <div className="flex-1 overflow-hidden rounded border border-slate-200">
        <table className="w-full border-collapse text-left">
          <thead className="border-b border-slate-200 bg-slate-50/90">
            <tr>
              {colList.map((colIdx) => (
                <th key={colIdx} className="h-9 border-r border-slate-200/80 px-3 py-2 last:border-r-0">
                  <div
                    className={cn(
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {rowList.map((rowIdx) => (
              <tr key={rowIdx} className="hover:bg-slate-50/40">
                {colList.map((colIdx) => (
                  <td key={colIdx} className="border-r border-slate-100 px-3 py-2.5 last:border-r-0">
                    <div
                      className={cn(
