...
describe('Vietnamese shared date input', () => {
  it('keeps input text separate from the border color token', () => {
    render(<Input type="text" aria-label="Tên" value="Nội dung" readOnly />)
    expect(screen.getByRole('textbox', { name: 'Tên' })).not.toHaveClass('text-input')
  })

  it('shows Vietnamese date text while preserving ISO change values', async () => {
    const onChange = vi.fn()
    render(<Input type="date" value="2026-08-10" onChange={onChange} aria-label="Tuần bắt đầu" />)

    const input = await screen.findByRole('textbox', { name: 'Tuần bắt đầu' })
    expect(input).toHaveValue('10/08/2026')
    expect(input).toHaveAttribute('data-date-locale', 'vi-VN')
    expect(input).not.toHaveClass('text-input')
  ...
  it('uses Vietnamese calendar labels', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Mở lịch chọn ngày' }))
  ...
  it('normalizes a Sunday to the following Monday for week-start fields', async () => {
    const onChange = vi.fn()
