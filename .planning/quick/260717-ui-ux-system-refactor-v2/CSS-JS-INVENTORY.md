...
- Approval queue next-action text now uses the shared status formatter; raw `PENDING`/other backend values remain in the record and are only translated at render time. Evidence: `638b3a5`, unit `88/88`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- Warehouse and Reports handoff links now use the shared workflow formatter for visible next-action copy; link targets and raw row data remain unchanged. Evidence: `fe0b57a`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- Visible report/purchasing vocabulary now uses Vietnamese explanations for `NVL` and `Handoff`; technical tab ids, link targets and API fields remain unchanged. Evidence: `da73ff5`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- Remaining visible `workflow` wording in the shared document rail, Reports title and Warehouse empty-state explanation now uses Vietnamese operational language. DOM/CSS/API/action contracts remain unchanged. Evidence: `4cf73d8`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build; `DocumentRail` remains protected as a HIGH-impact shared component.
- The Admin statistics table now explains `Handoff` as `Chuyển xử lý`; only the presentation header hunk was staged from the dirty route. No import, BOM, link or mutation logic was touched. Evidence: `c689af0`, lint/build, controls `19/19`, smoke `15/15`, staged detect check.
- The clean application shell now explains `Tổng quan workflow` as `Tổng quan vận hành`; internal route context keys remain technical and unchanged. Evidence: `d4d48fd`, lint/build, UI audit desktop/mobile `4/4`; staged detection classified the shared shell as MEDIUM.
...
- Purchasing orders no longer uses the legacy list hook with `pageSize: 100`; it consumes the bounded page response already used by the supplier surface. The table remains scroll-contained and paginated; mutation/action ownership is unchanged. Evidence: `9fa6175`, unit `88/88`, controls `19/19`, smoke `15/15`, lint/build.
- Purchasing document-rail loading is bounded to 20 records while the visible rail remains limited to four; this removes an oversized preview request without changing document actions or layout rules. Evidence: `4de10b0`, controls `19/19`, smoke `15/15`, lint/build.
- Approval document loading and Chef document/stock preview loading are bounded to 20 records; the visible consumers remain local-window surfaces. Chef issue loading was retained because it supports shift filters and counts. Evidence: `eda4f22`, unit `88/88`, controls `19/19`, smoke `15/15`, lint/build; serial browser gates pass.
- Warehouse document preview loading is bounded to 20 records; demand, issue and cursor movement requests remain unchanged because they support operational actions, counts or navigation. Evidence: `f4b3db8`, unit `88/88`, controls `19/19`, smoke `15/15`, lint/build.
- Warehouse demand no longer fetches the unbounded `limit: 100` collection; it consumes the existing page-number endpoint with 8 rows per page and a visible `PaginationBar`. The `shortageCount` aggregate drives KPI/exception state while the current page drives the summary list. Evidence: `506b32f`, unit `88/88`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- Approval purchase-request history list no longer fetches the unbounded collection into a fixed-height scroll box; it uses the page-number endpoint with 8 rows per page and a visible `PaginationBar`. Selection/history/mutation contracts are unchanged. Evidence: `951a7c7`, unit `88/88`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- JavaScript feedback scan classification is now explicit: no `window.alert/confirm/prompt` or user-facing console feedback remains. The `ToastProvider` timer is retained only for typed toast auto-dismiss, and `SessionTimeoutModal` keeps its short redirect timer for session-expiry security flow; neither is dead UI feedback logic.
- Purchasing current-stock loading is bounded to 20 because the route consumes it only to derive distinct warehouse options for receipt assignment; no stock table, mutation payload or document surface uses the full collection. Evidence: `2b9a205`, unit `88/88`, controls `19/19`, smoke `15/15`, UI audit `4/4`, lint/build.
- Residual large-query inventory remains explicit: `workflowApi.ts` shared `useWorkflowOverview` requests and `queryWithLimit` are dirty/shared; `WeeklyMenuPage.tsx` contains the 100/500 demand queries beside user-owned weekly-production/import work; `AdminDataPage.tsx` contains the audit `limit: 100` beside user-owned BOM/admin work. Chef/Warehouse kitchen-issue collections and Approval Rules employee options remain bounded by their KPI/filter or catalog contracts. No residual was edited without ownership/API-contract evidence.
- Weekly Menu `limit: 500` and Chef/Warehouse kitchen-issue limits are retained pending an aggregate API contract: the consumers calculate material totals, shortage/ready state or pending-by-shift status from raw rows. A client-only pager would produce incomplete operational numbers, so these are tracked as API/ownership follow-up rather than cosmetic limit changes.
- Task 4.5.3 is now the explicit follow-up for those residual collections: backend aggregate page DTOs, RTK Query mapping, route migration and equality/transition tests must land before the remaining raw `limit: 500/100` requests can be removed safely.
- `StockMovementTable` no longer carries an inline `22px` copy-button geometry override; the existing `.ipc-document-copy-button` CSS rule remains the single size owner at `28px`. The shared component's clipboard toast, focus style, row actions and public props are unchanged. Evidence: `898c647`, HIGH impact with four callers, full serial gates pass.
