...
- Cursor pagination convergence: `fd6def4` extracts Reports' cursor-only control into `CursorPaginationBar`; it preserves cursor navigation and deliberately exposes no total-page/local-total fields. Reports now shares the common pagination surface. Unit `70/70`, lint, build and UI audit `2/2` pass; staged detection covered one Reports flow at MEDIUM scope.
- Weekly-menu pagination convergence: `2854243` replaces the purchase-summary route-local footer with canonical `PaginationBar`, preserving zero/one/many-page behavior through the shared controller. The dirty route was staged hunk-by-hunk; BOM/template/production-plan hunks remain unstaged. Unit `70/70`, lint, build and UI audit `2/2` pass; GitNexus direct route impact was LOW, while staged detection returned `No changes detected` for the isolated hunk.
- Grouped-page navigation convergence: `d844ed9` adds `PageStepper` for production-plan pages grouped by service day. It preserves the domain summary outside the stepper and uses shared accessible controls without pretending grouped pages are row ranges. Unit `72/72`, lint, build and UI audit `2/2` pass; user-owned WeeklyMenu feature hunks remain unstaged.
