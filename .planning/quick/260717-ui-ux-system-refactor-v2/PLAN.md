...
Reference defect acceptance (ảnh tham chiếu màn Thu mua):

- Sidebar phải giữ một vùng điều hướng duy nhất, không lặp menu/user panel; vùng tài khoản và đăng xuất neo ổn định ở cuối sidebar mà không tạo khoảng trắng/layout drift bất thường.
- Nội dung chính phải có một page header và một command region; bảng lớn phải nằm trong table viewport có boundary/overflow riêng, không kéo dài document tới khi tải hết dữ liệu.
- Mọi bảng/collection dài phải chứng minh được contract lazy/server pagination hoặc cursor pagination; không dùng pager chỉ cắt mảng sau khi đã fetch toàn bộ collection.
- Ở viewport 320–390px, action không bị khuất, form tự stack, table cuộn ngang bên trong vùng được đặt tên và document không phát sinh horizontal overflow hoặc nested scroll cạnh tranh.
- Technical keys/statuses chỉ xuất hiện khi cần traceability và phải có nhãn giải thích; required/reason/error phải được hiển thị bằng copy tiếng Việt rõ nghĩa.
- Mỗi route sau khi sửa phải có evidence trước/sau về geometry và feedback surface; không xem việc static scan không còn `window.alert` là đủ để đóng task.
...
- Operational vocabulary cleanup now removes the remaining visible `workflow` wording from the shared `DocumentRail` default/ARIA label and from Reports/Warehouse explanatory copy. DOM structure, CSS classes, API values, route links, document actions and raw workflow records are unchanged. `DocumentRail` impact was HIGH (6 callers, 4 flows); Reports and Warehouse were LOW/unindexed. Evidence: lint/build, controls `19/19`, smoke `15/15`, UI audit `4/4`, staged detection MEDIUM for the shared flow; commit `4cf73d8`.
- Admin statistics now labels its handoff column as `Chuyển xử lý` instead of raw English `Handoff`. Only the isolated header-copy hunk was staged from the dirty `AdminDataPage`; BOM/import logic, route actions, links and all other user-owned hunks remain unchanged. Evidence: `AdminDataPage` impact LOW, lint/build, controls `19/19`, smoke `15/15`, staged diff check and `detect_changes`; commit `c689af0`.
- The shared shell now presents the overview context as `Tổng quan vận hành` instead of `Tổng quan workflow`. Only visible copy changed; route context keys, lane resolution, status tone and action state remain unchanged. Source impact was LOW, while staged detection conservatively reported MEDIUM across four App flows. Evidence: lint/build, UI audit desktop/mobile `4/4`; commit `d4d48fd`.
- Dashboard semantic-copy cleanup remains gated: its 479-line user-owned rewrite is one Git hunk, so staging a seemingly small label change would also stage unrelated feature work. The attempted copy-only edit was reverted to preserve the baseline; R82 records the required ownership handoff before Dashboard layout/copy changes or snapshot updates.
- R61 verification was re-audited after a fresh GitNexus index: the backend C# symbols remain outside the graph (`ApprovalInboxService` and `GetPendingPageAsync` return UNKNOWN impact), so no service code was changed. Existing focused ApprovalInbox tests pass `2/2` with `--no-build`; the normal test command is currently blocked by the running API process locking `apphost.exe`. Same-code sibling and production-shaped large-volume evidence is still required.
- The clean table viewport stylesheet now uses the canonical `--ipc-slate-50` token for sticky table headers instead of a duplicate `#f8fafc` literal. Sticky positioning, table scroll boundaries and route markup are unchanged. GitNexus has no CSS selector mapping, so staged detection returned `No changes detected`; evidence is the one-line source diff plus lint/build and UI audit desktop/mobile `4/4`. Commit: `b2d0ac7`.
- Shared `uiCopy` no longer carries three zero-reference keys (`bomCanonical`, `sourceHash`, `sla`). All referenced labels and rendered copy remain unchanged; this removes dead JavaScript configuration without changing the feedback/modal or route contracts. Evidence: static reference scan, lint/build, unit `88/88`; commit `db58c11`.
- `DemandSummary` now renders `Tạo lại nhu cầu` instead of the raw mixed-language `Tạo lại demand` action. The match still accepts the backend phrase, while raw action values, route links, callbacks and pagination remain unchanged. `DemandSummary` impact was LOW with three direct callers; staged detection classified the shared flow as MEDIUM. Evidence: unit `88/88`, controls `19/19`, lint/build; commit `63050c7`.
- Purchasing orders view no longer requests all purchase requests with `pageSize: 100`; it reuses the existing server page contract (`pageNumber`, `pageSize: 8`) and keeps the current `PaginationBar`. This removes the long-table fetch path shown by the reference UI without changing order mutations or payloads. `PurchasingPage` source impact was LOW; staged detection reported MEDIUM for its flow. Evidence: unit `88/88`, controls `19/19`, smoke `15/15`, lint/build; commit `9fa6175`.
- Purchasing’s document rail preview is now bounded to 20 records instead of 100. The rail still renders its local four-document window, document filtering and actions are unchanged, and no historical table contract was altered. Evidence: controls `19/19`, smoke `15/15`, lint/build, staged detection for Purchasing; commit `4de10b0`.
