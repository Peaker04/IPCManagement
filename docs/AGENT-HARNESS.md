---
title: Pi-first runtime and skill adapter
status: canonical-adapter
owner: GSD
---
# Runtime and skill adapter

This file owns runtime integration, not a second delivery process. Lanes, feedback loops and Done remain in [Lean standard](LEAN-DELIVERY-AND-DEBUGGING-STANDARD.md); task state stays in GSD. Startup rules are in [AGENTS.md](../AGENTS.md). No provider keys or live database values belong here.

## 1. Runtime contract

- Pi CLI is primary. Its configured API/provider/model supplies inference, not a second CLI or its MCP registry.
- Codex CLI is secondary for an explicitly needed capability. Before handoff verify its tools, cwd, permissions and output path; provide a minimal task packet. Parent Pi remains the GSD state owner. Do not silently route to another runtime/provider or copy credentials.
...
- Native GSD for Pi is installed under `~/.pi/agent/gsd-core` with its `.gsd-runtime` set to `pi`; `/gsd` is registered by `~/.pi/agent/extensions/gsd.js`. The older Codex core remains the secondary-adapter installation; do not route Pi through it or delete shared historical assets.
...
The imported ~/.pi/agent/agents/gsd copies are project-disabled until individually adapted and tested. Keep their source for reference/Codex; do not auto-enable them after an update. A new imported name must be reviewed before use. For ordinary scoped work use inline execution or the bounded builtin roles below, not a 34-agent manual port.
...
- Default project discipline: karpathy-guidelines, diagnosing-bugs, tdd, ui-styling, sketch-findings-ipcmanagement. Front-End-Checklist is already a project Pi skill.
