...
| Reports: price/demand/stock/data-quality | tab switch, search, sort, pagination, export/action | `tab-switch`, `search-keystroke`, `table-sort`, `row-action`, `modal-open` | Add explicit pagination/export interaction IDs where the control is user-visible |
| Admin Data: BOM/contracts/cleanup/inventory/statistics/employees/audit | tab switch, search, sort, pagination, row action, modal/submit | full catalog | Keep technical diagnostics out; measure only user-facing actions |
