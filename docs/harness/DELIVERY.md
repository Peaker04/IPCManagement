...
[`documentation map`](../README.md); không tự mở một audit/checklist cũ làm process song song.
...
## 10. Ngân sách, context và subagent
...
- Một mục tiêu tuần tự dùng một PLAN và một CHECKLIST có ID; task nhỏ không cần tạo mới trừ khi người dùng
  yêu cầu hoặc cần resume nhiều bước. Checklist là nơi duy nhất giữ trạng thái từng bước; MEMORY/STATE trỏ link.
- Trước mỗi bước đọc checkpoint và owner liên quan, không đọc lại toàn audit/transcript. Chỉ một bước đang làm.
- Sau mỗi bước đã kiểm chứng, trước command dài hoặc đổi context: ghi paths đã đổi, command/exit/result,
  giới hạn evidence, blocker và đúng hành động tiếp theo. Ghi vị trí backup nếu có thay đổi rules/config.
- Theo dõi áp lực context thực tế (khối lượng còn lại, chi phí gate dài, compaction và khả năng review an toàn),
  không phát minh API hay ngưỡng token 250k/300k mà runtime không hỗ trợ. Khi áp lực tăng, hoàn tất wave đang
  kiểm chứng rồi checkpoint; không mở thêm scope/worker.
- Handover fresh-session phải ghi: goal/wave hiện tại, commit đã hoàn tất, branch/HEAD + status/index, file/hunk
  task-owned so với inherited dirt, đúng bước/command kế tiếp, test/evidence và giới hạn, process thuộc run,
  blocker và pointer authority. MEMORY chỉ trỏ checklist/handover, không copy narrative.
- Nếu có quyền commit, stage đúng task-owned hunks, kiểm `git diff --cached`, chạy gate wave, commit local và xác
  nhận index sạch; không push. Sau đó bắt đầu wave tiếp theo trong fresh child/session bằng packet tối thiểu hoặc
  dừng với resume pointer/command. Phiên mới phải đối chiếu checkpoint với source/git/runtime và không được giả
  định đã kế thừa phần chat chưa persist.
- Không nạp toàn skill catalog vào child hoặc fork lịch sử dài mặc định. Fresh packet gồm task ID, cwd,
  allowed files, required reads, acceptance, forbidden actions, test và output. Fork chỉ khi cần lịch sử có lý do.
- Tách runner/parser result, test/evidence result và task verdict. Report sai schema không phải regression fail;
  runner thành công không phải task PASS. Required evidence thiếu thì bước đó chưa hoàn tất.
...
- Launch phải có runtime/tool timeout rõ, phù hợp command đo được và nhỏ hơn session budget; không thêm
  settings key mà runtime bỏ qua. Không hard-block read tools của writer giữa chừng bằng tool budget.
  Broad command từng treo không được chạy lại không giới hạn. Khi timeout lưu command, thời điểm và process
  thuộc run; không khẳng định không còn process nếu chưa kiểm tra.
