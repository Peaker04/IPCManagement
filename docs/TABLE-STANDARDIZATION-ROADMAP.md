...
| --- | --- | --- | --- |
| W7-RUNTIME-01 | Runtime/FE | Static check pass: 9 routes, 27 targets, 8 interactions; production build/lint pass. With `noticePlacement="overlay"` in the current working tree and production bundle pointed at `5262`, H.1 `schedule` rerun is gradable with integrity `0`, `CLS window=0.0581`, `CGR=0.0878`, growth `0`, and `deltaTop=79px` (all three verdicts DAT). `VITE_ENABLE_MOCK_LOGIN` is DEV-only. A real admin credential attempt against the local lane timed out during `bootstrapAuth` after 60s, so report targets still lack valid credential/API-lane evidence. | Commit the scoped implementation atomically; reduce remaining `deltaTop` or document it as bounded non-blocking shift with evidence; rerun report targets on an authenticated API lane; then run `--load`, `--inp`, `--overflow` across all five desktop viewports before closing Wave 7. |
