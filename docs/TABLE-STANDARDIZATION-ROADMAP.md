...
| --- | --- | --- | --- |
| W7-RUNTIME-01 | Runtime/FE | Static check pass: 9 routes, 27 targets, 8 interactions; production build/lint pass. Authenticated production-preview H.1 `schedule` rerun is now gradable: mount `8830.9ms`, settled `9812.7ms`, one 48px data row at both samples, integrity `0`, `CLS window=0.0007`, `CGR=0/900`, growth `0/97`, and `deltaTop=0`. Overflow rerun passes all four configured probe viewports; 1280px retains two expected ellipsis WARN and one scroll INFO, with no visual clipping FAIL. The earlier 60s timeout is dispositioned as Vite dev-module delivery under throttle, not credential/auth failure. | Extend authenticated production-build evidence from `schedule` to the remaining route/tab targets, run INP decomposition, and prove loading/ready plus conditional empty/error owners before closing Wave 7. |
