...
| --- | --- | --- | --- |
| W7-RUNTIME-01 | Runtime/FE | Full authenticated production-preview H.1 load matrix completed for all 27 original targets with integrity `0`; 7 targets were gradable and 20 correctly remained `N/A` because the configured anchor/frame or settled data was absent. Six gradable targets passed all three verdicts. `warehouse-movement` initially exposed a real shift: 8 data rows/429px at `t_0` settled to 1 row/94px (`deltaTop=-551px`, `CGR=0.6122`). The route now reserves `27rem` geometry for both asynchronous warehouse tables; focused contract suite is 6/6 green. Targeted production-preview rerun is green: 8 rows and 429px at both samples, `deltaTop=0`, `CGR=0`, growth `0`, integrity `0`, all three verdicts `DAT`. Probe INP was corrected to activate the target tab per cell, scope controls to the active panel, add semantic pagination/export/decision owners, include Purchasing (30 targets total), and support `PROBE_INTERACTIONS` batching without changing the default full matrix. Corrected five-repeat warehouse search and tab-switch cells are both DAT, presentation-dominated, and not processing debt; conditional export/purchasing actions are recorded as justified `N/A` where owners are not rendered. | Run remaining corrected INP cells at five repeats, resolve the remaining N/A selector/conditional-owner gaps, and prove loading/ready plus conditional empty/error owners before closing Wave 7. |
