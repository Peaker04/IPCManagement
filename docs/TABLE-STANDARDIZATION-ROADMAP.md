...
| --- | --- | --- | --- |
| W7-RUNTIME-01 | Runtime/FE | Full authenticated production-preview H.1 load matrix completed for all 27 targets with integrity `0`; 7 targets were gradable and 20 correctly remained `N/A` because the configured anchor/frame or settled data was absent. Six gradable targets passed all three verdicts. `warehouse-movement` is a real blocker: at `t_0` it had 8 data rows, 8×49px, frame `clientHeight=429`, then settled to 1×48px, `clientHeight=94`, `deltaTop=-551px`, `CGR=0.6122` against `CGR_MAX=0.1`; CLS window was `0.0817` and scroll growth `-0.7809`. This is a data/query transition, not a threshold or renderer-policy dispute. The earlier dev timeout remains dispositioned as Vite module delivery under throttle, not credential/auth failure. | Trace and fix the warehouse movement query/data transition, then rerun the full matrix; separately resolve the 20 N/A selector/conditional-owner gaps, run INP decomposition, and prove loading/ready plus conditional empty/error owners before closing Wave 7. |
