...
# Chọn file mẫu theo môi trường; ví dụ production:
...
Backend đã cấu hình CORS cho phép FE truy cập trong development mode. Xem `backend/src/IPCManagement.Api/appsettings.json` > `Cors.AllowedOrigins`.

### Production readiness

Không dùng trực tiếp `backend/src/IPCManagement.Api/appsettings.json` development cho môi trường thật. Sao chép `backend/src/IPCManagement.Api/appsettings.Production.example.json` thành cấu hình production hoặc set các biến môi trường tương ứng:
...
| --- | --- | --- |
| `Development` | `backend/src/IPCManagement.Api/appsettings.json` + `backend/src/IPCManagement.Api/appsettings.Development.json` | Máy dev local, Swagger bật, CORS mở trong development. |
| `Demo` | `backend/src/IPCManagement.Api/appsettings.Demo.example.json` | Demo nội bộ với database/demo secret riêng. Sao chép thành file runtime có suffix `Demo` trong thư mục backend trước khi chạy. |
| `Lan` | `backend/src/IPCManagement.Api/appsettings.Lan.example.json` | Chạy API trong mạng LAN bằng IP/host nội bộ. Sao chép thành file runtime có suffix `Lan` trong thư mục backend trước khi chạy. |
| `Production` | `backend/src/IPCManagement.Api/appsettings.Production.example.json` | Triển khai thật, dùng host/domain thật và secret mạnh. Sao chép thành file runtime có suffix `Production` trong thư mục backend hoặc set env vars. |

Ví dụ chạy LAN từ repo root sau khi đã thay toàn bộ `CHANGE_ME_*`, IP, host, database và secret trong file runtime LAN của backend:
...
Các script migration/seed/release được khai báo trong `package.json` nhưng không phải tất cả file PowerShell đích đều có trong checkout hiện tại. Chỉ chạy các lệnh đó sau khi xác nhận file tương ứng đã có trong `scripts/`; script E2E đang có trong repo là `scripts/Invoke-Iter1HappyPathE2E.ps1`.
...
- [Bắt đầu dự án](docs/GETTING-STARTED.md)
...
Backend coverage runs through `backend/coverage.runsettings`, which excludes EF migration files from the report so the baseline focuses on application code. The generated backend report is written under `backend/TestResults/CoverageReport/` after the coverage command completes.

Frontend unit tests use Vitest + React Testing Library. The generated frontend report is written under `frontend/coverage/` after the coverage command completes.
