[CmdletBinding()]
param(
    [int]$FrontendPort = 3036,
    [int]$ApiPort = 8036,
    [string]$Database = 'ipc_lane7',
    [string]$ArtifactRoot = '.artifacts/shipyard-live/phase05-golden-demo-runtime'
)

$ErrorActionPreference = 'Stop'
if ($Database -cne 'ipc_lane7') { throw 'Runtime may target exact ipc_lane7 only.' }
...
if ($connection -notmatch '(?i)(Database|Initial Catalog)=ipc_lane7(?:;|$)') { throw 'Database fence failed.' }
...
if (-not $ready -or $frontendStatus -ne 200) {
    ...
    throw "Runtime startup failed. Inspect $artifact."
}
