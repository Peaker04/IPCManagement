...
if ($AuditOnly) {
    $checks = @(
        @{ Name = "backend restore command"; Path = "package.json"; Command = "dotnet restore backend/src/IPCManagement.Api/IPCManagement.Api.csproj" },
        @{ Name = "frontend restore command"; Path = "package-lock.json"; Command = "npm ci" },
        ...
        @{ Name = "seed mode script"; Path = "scripts/Invoke-Iter1SeedMode.ps1"; Command = "powershell -ExecutionPolicy Bypass -File scripts/Invoke-Iter1SeedMode.ps1 -Mode DemoReset -AuditOnly" }
    )
    ...
    $commands = @(
        @{ Name = "backend restore"; Command = "dotnet restore backend/src/IPCManagement.Api/IPCManagement.Api.csproj" },
        @{ Name = "frontend restore"; Command = "npm ci" },
    ...
    if (@($results | Where-Object { $_.Status -ne "PASS" }).Count -gt 0) {
        ...
        if ($RunSeedReset) {
            $seedCommand = "powershell -ExecutionPolicy Bypass -File scripts/Invoke-Iter1SeedMode.ps1 -Mode DemoReset -BaseUrl `"$BackendBaseUrl`""
            $results.Add((Invoke-GateCommand -Name "seed reset" -Command $seedCommand))
        }
        else {
            $results.Add((New-GateResult `
                -Name "seed reset" `
                -Command "powershell -ExecutionPolicy Bypass -File scripts/Invoke-Iter1SeedMode.ps1 -Mode DemoReset -BaseUrl `"$BackendBaseUrl`"" `
                -Status "BLOCKED" `
