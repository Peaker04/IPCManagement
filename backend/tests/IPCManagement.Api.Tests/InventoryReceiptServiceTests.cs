    ...
    public async Task CreateFromPurchaseRequestAsync_Should_Track_ReceivedQuantity_ByPurchaseLine()
    {
        ...
        var dto = new CreateInventoryReceiptFromPurchaseDto
        {
            ...
            [
                new()
                {
                    PurchaseRequestLineId = IPCManagement.Api.Helpers.GuidHelper.ToGuidString(firstPurchaseLineId),
                    ReceivedQty = 50,
                    UnitId = IPCManagement.Api.Helpers.GuidHelper.ToGuidString(unitId)
                },
                new()
                {
                    PurchaseRequestLineId = IPCManagement.Api.Helpers.GuidHelper.ToGuidString(secondPurchaseLineId),
                    ReceivedQty = 50,
                    UnitId = IPCManagement.Api.Helpers.GuidHelper.ToGuidString(unitId)
                }
            ]
