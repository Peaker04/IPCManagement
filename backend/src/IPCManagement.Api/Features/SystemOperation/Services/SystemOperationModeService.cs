...
public sealed class SystemOperationModeService(IpcManagementContext context, SystemOperationModeGuard guard, IEfTransactionRunner transactions)
{
    ...
    public async Task<SystemOperationModeDto> ChangeAsync(ChangeSystemOperationModeRequest request, string actorId, CancellationToken cancellationToken = default)
    {
        ...
        var resultingVersion = request.ExpectedVersion;
        return await transactions.ExecuteAsync(
            async operationToken =>
            {
                ...
                var reasonRequired = await HasWorkInProgressAsync(operationToken);
