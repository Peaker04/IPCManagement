...
public sealed class ReconciliationBatchesController : ControllerBase
{
    ...
    public ReconciliationBatchesController(ReconciliationBatchService service, ReconciliationCompletionService completion, ICurrentUserService currentUser)
    {
        this.service = service;
        this.completion = completion;
        this.currentUser = currentUser;
    ...
    private ReconciliationQuantityImportService RequiredQuantityImports() => quantityImports ?? throw new InvalidOperationException("Dịch vụ cam kết số suất chưa được cấu hình.");
}
